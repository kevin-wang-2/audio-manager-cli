...
//	#define ASIO_DRIVER_NAME    "ASIO Multimedia Driver"
	#define ASIO_DRIVER_NAME    "O DESU ASIO LINK PRO"
#elif MAC
...
void copyInt32Buf(int *buf, void *dest, int size) {
    constexpr auto overflow = 32 - destSize;
    constexpr auto blockCnt = destSize / 8; // Block count of character
    if (overflow == 0) {
        // Target Bit Depth = Calculated Bit Depth
        memcpy(dest, buf, size * 4);
    } else {
        // Target Bit Depth is smaller, resize
        char *destMem = (char *)dest;
        for (int i = 0; i < size; i++) {
            int val = buf[i] >> overflow; // Resize
            for (int offset = 0; offset < blockCnt; offset++) { // Copy to destination
                destMem[offset] = *((char*)(&val) + offset);
            }
            destMem += blockCnt;
        }
    }
...
{	// the actual processing callback.
	...
	// Calculate Buffer
	int buf[2][buffSize];
    constexpr double omega = 440 * 2 * 3.1415926535897932384626; // A
    double sample = 1000000.0 / 48000.0;
    static int counter = 0;
    static double startTime = asioDriverInfo.nanoSeconds / 1000;


    double microSecs = counter * sample * buffSize;

    // printf("ns: %lf-%lf\n", sin((nanoSecs / 1000) * omega / 1000000.0),sin((nanoSecs / 1000 + sample * buffSize) * omega / 1000000.0));

    printf("%d -  ns: %lf, st: %lf, delta: %lf\n", counter,  asioDriverInfo.nanoSeconds / 1000, microSecs + startTime, asioDriverInfo.nanoSeconds / 1000 - microSecs - startTime);

    for(int i = 0; i < buffSize; i++)
    {
        buf[0][i] = 2147483647 * sin((microSecs + sample * i) * omega / 1000000.0);
        buf[1][i] = 2147483647 * sin((microSecs + sample * i) * omega / 1000000.0);
    }

    counter++;

	// Translate Buffer
    for (int i = asioDriverInfo.inputBuffers; i < asioDriverInfo.inputBuffers + 2; i++)
	{
		if (asioDriverInfo.bufferInfos[i].isInput == false)
		{
			...
			{
            case ASIOSTInt16LSB:
			    copyInt32Buf<16>(buf[i - asioDriverInfo.inputBuffers], asioDriverInfo.bufferInfos[i].buffers[index], buffSize);
				break;
            case ASIOSTInt24LSB:		// used for 20 bits as well
                copyInt32Buf<24>(buf[i - asioDriverInfo.inputBuffers], asioDriverInfo.bufferInfos[i].buffers[index], buffSize);
				break;
            case ASIOSTInt32LSB:
                copyInt32Buf<32>(buf[i - asioDriverInfo.inputBuffers], asioDriverInfo.bufferInfos[i].buffers[index], buffSize);
				break;
...
int main(int argc, char* argv[])
{
    ...
    char drivers[32][1024];
    char *p_drivers[32];
    for (int i = 0; i < 32; i++) p_drivers[i] = drivers[i];
    int cnt = asioDrivers->getDriverNames(p_drivers, 32);
		...
		if (ASIOInit (&asioDriverInfo.driverInfo) == ASE_OK)
		{
			...
			if (init_asio_static_data (&asioDriverInfo) == 0)
			{
				...
				if (create_asio_buffers (&asioDriverInfo) == ASE_OK)
				{
                    ...
                    for (channel = 0; channel < asioDriverInfo.inputChannels; channel++) {
                        printf("%d: %s\n", channel, asioDriverInfo.channelInfos[channel].name);
                    }

                    printf("Output Channels: \n");

                    for (; channel < asioDriverInfo.inputChannels + asioDriverInfo.outputChannels; channel++) {
                        printf("%d: %s\n", channel - asioDriverInfo.inputChannels, asioDriverInfo.channelInfos[channel].name);
                    }
